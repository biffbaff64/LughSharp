...
public interface IAudio
{
    ...
    /// <summary>
    ///     Creates a new <see cref="ISound" /> which is used to play back audio effects such as gun shots or explosions. The
    ///     Sound's audio data
    ///     is retrieved from the file specified via the {@link FileHandle}. Note that the complete audio data is loaded into
    ///     RAM. You
    ...
    /// <summary>
    ///     Creates a new <see cref="IMusic" /> instance which is used to play back a music stream from a file. Currently
    ///     supported formats are
    ///     WAV, MP3 and OGG. The Music instance has to be disposed if it is no longer used via the {@link Music#dispose()}
    ///     method.
