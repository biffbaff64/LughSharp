...
public abstract class InputStream : ICloseable
{
    ...
    /// </exception>
    public virtual int Read( ref byte[] b ) => Read( b, 0, b.Length );
    ...
    ///     </para>
    ///     <para>
    ///         If <tt>len</tt> is zero, then no bytes are read and
    ///         <tt>0</tt> is returned; otherwise, there is an attempt to read at
    ...
    ///     </para>
    ///     <para>
    ///         The first byte read is stored into element <tt>b[off]</tt>, the
    ...
    ///     </para>
    ///     <para>
    ///         In every case, elements <tt>b[0]</tt> through
    ...
    /// </exception>
    public virtual void Reset() => throw new IOException( "mark/reset not supported" );
