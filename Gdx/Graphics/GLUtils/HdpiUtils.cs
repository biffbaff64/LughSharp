...
public class HdpiUtils
{
    ...
    /// </summary>
    public static int ToLogicalX( int backBufferX ) => ( int )( ( backBufferX * Gdx.Graphics.Width ) / ( float )Gdx.Graphics.BackBufferWidth );

    /// <summary>
    ///     Convers an y-coordinate given in backbuffer coordinates to
    ///     logical screen coordinates
    /// </summary>
    public static int ToLogicalY( int backBufferY ) => ( int )( ( backBufferY * Gdx.Graphics.Height ) / ( float )Gdx.Graphics.BackBufferHeight );

    /// <summary>
    ///     Converts an x-coordinate given in logical screen coordinates to
    ///     backbuffer coordinates.
    /// </summary>
    public static int ToBackBufferX( int logicalX ) => ( int )( ( logicalX * Gdx.Graphics.BackBufferWidth ) / ( float )Gdx.Graphics.Width );

    /// <summary>
    ///     Convers an y-coordinate given in backbuffer coordinates to
    ///     logical screen coordinates
    /// </summary>
    public static int ToBackBufferY( int logicalY ) => ( int )( ( logicalY * Gdx.Graphics.BackBufferHeight ) / ( float )Gdx.Graphics.Height );
}
